...
class WordOrder:
    ...
    def __init__(self, sentence):
        ...
        self.verb     = get_root(sentence)

        self.subject  = get_subject(self.verb, sentence)
        self.object   = get_object(self.verb, sentence)
        self.comp     = get_complement(self.verb, sentence)
        self.conj     = get_conjunct(self.verb, sentence)
        self.aux      = get_auxiliary(self.verb, sentence)

        self.order    = get_order(self.subject, self.object. self.verb)

    # Check if the verb has a nominal subject.
    def get_subject(verb_index, sentence):

        subj = {}

        for tok in sentence:
            if tok[6] == verb_index and tok[7].startswith('nsubj'):
                subj['nsubj'] = int(tok[0])

        return subj

    # Check if the verb has an object.
    def get_object(verb_index, sentence):
    ...
    def get_complement(verb_index, sentence):
    ...
    def get_conjunct(verb_index, sentence):
    ...
    def get_auxiliary(verb_index, sentence):
        aux = {}
...
# Find constituents and pass to determine_order
    def to_dict(verb_index, sentence):

        verb_index_adjust = int(verb_index)
        ...
        order_info.update({'verb': sentence[verb_index-1][1], 'verb_lemma': sentence[verb_index-1][2], 'verb_id': sentence[verb_index-1][0]})
